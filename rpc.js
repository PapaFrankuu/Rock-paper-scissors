const score = JSON.parse(localStorage.getItem("score")) || {
  ...
  const recentMoves = JSON.parse(localStorage.getItem("recentMoves")) || [];
  ...
  function reset() {
    ...
    localStorage.setItem("recentMoves", JSON.stringify(recentMoves));
  ...
  function pickComputerMove() {
    ...
    if (recentMoves.length >= 3) {
      ...
      const last3Same = recentMoves.slice(-3).every((move) => move === last);
      if (last3Same) {
        computerMove = getCounterMove(last);
        return;
      }
    }
  

    const total = Object.values(playerHistory).reduce((a, b) => a + b, 0);
    if (total === 0) {
      computerMove = getRandomMove();
      return;
    }
    ...
    computerMove = getCounterMove(predicted);
  }
